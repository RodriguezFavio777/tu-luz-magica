...
export default function LoginPage() {
    ...
    const handleRecoverPassword = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.auth.resetPasswordForEmail(email, {
            redirectTo: `${location.origin}/actualizar-password`,
        })
        ...
        return (
            <main className="min-h-screen bg-[#0a080c] flex items-center justify-center p-4 relative overflow-hidden pt-32">
                <div className="relative z-10 w-full max-w-md bg-[#1d1520] border border-white/10 p-8 rounded-3xl shadow-2xl text-center">
                    ...
                    <button onClick={() => { setRecoverSuccess(false); setIsRecovering(false) }} className="w-full bg-primary hover:bg-primary-hover text-white font-bold py-4 rounded-xl shadow-lg transition-all active:scale-95">
            ...
            <div className="relative z-10 w-full max-w-md bg-[#1d1520] border border-white/10 p-8 rounded-3xl shadow-2xl">
                <div className="text-center mb-8">
                    ...
                    <p className="text-white/50 text-sm">
                        {isRecovering ? 'Ingresa tu correo y te enviaremos un enlace para restablecer tu contraseña.' : 'Ingresa a tu cuenta para gestionar tus pedidos y citas.'}
                    </p>
                </div>
                ...
                {isRecovering ? (
                    <form onSubmit={handleRecoverPassword} className="space-y-5">
                        ...
                        >
                            {loading ? <Loader className="w-5 h-5 animate-spin" /> : 'Enviar enlace mágico'}
                        </button>

                        <div className="mt-4 text-center">
                            <button type="button" onClick={() => { setIsRecovering(false); setError(null) }} className="text-white/40 hover:text-white text-sm transition-colors">
                                Volver a Iniciar Sesión
                            </button>
                        </div>
                    </form>
                    ...
                    <>
                        <form onSubmit={handleLogin} className="space-y-5">
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center pr-1">
                                    ...
                                    <button type="button" onClick={() => { setIsRecovering(true); setError(null) }} className="text-xs text-primary hover:text-primary-hover tracking-wider">¿Olvidaste tu clave?</button>
                                </div>
                                <div className="relative">
                            ...
                            <button
                                ...
                                className="w-full bg-primary hover:bg-primary-hover text-white font-bold py-4 rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-95 flex items-center justify-center gap-2"
                            >
                                {loading ? <Loader className="w-5 h-5 animate-spin" /> : 'Ingresar'}
                            </button>
                        </form>
                        ...
                        <div className="relative my-8">
                            ...
                            <div className="relative flex justify-center text-xs uppercase tracking-wider">
                                <span className="bg-[#1d1520] px-4 text-white/30 font-bold">O continúa con</span>
                            </div>
                        </div>
                        ...
                        <div className="mt-8 text-center">
                            <p className="text-white/40 text-sm">
                                ...
                                <Link href="/registrarse" className="text-primary hover:underline font-bold">
                                    Regístrate aquí
                                </Link>
                            </p>
                        </div>
