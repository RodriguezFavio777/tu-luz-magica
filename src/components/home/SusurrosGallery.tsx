            ...
            <div className="container mx-auto px-6 max-w-7xl relative z-10">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        ...
                        <p className="text-white/30 animate-pulse uppercase tracking-widest text-xs">Escuchando los susurros...</p>
                    </div>
                ) : reviews.length > 0 ? (
                    <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
                        {reviews.map((item, idx) => (
                            ...
                            >
                                <div className="absolute top-6 right-8 text-white/5 opacity-50 group-hover:text-primary/20 transition-colors">
                                    <Quote className="w-12 h-12 rotate-180" />
                                </div>

                                <div className="flex items-center gap-3 mb-6">
                                    <div className="w-10 h-10 rounded-full bg-linear-to-br from-primary/20 to-secondary/20 border border-white/10 flex items-center justify-center text-primary font-bold">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-0.5 mt-0.5">
                                            {[...Array(5)].map((_, i) => (
                                                <Star key={i} className={`w-3 h-3 ${i < item.rating ? 'text-secondary fill-secondary' : 'text-white/10'}`} />
                                            ))}
                                        </div>
                                    </div>
                                </div>

                                <p className="text-white/70 text-base leading-relaxed font-body italic mb-6">
                                    &quot;{item.content}&quot;
                                </p>
                                ...
                                <div className="flex items-center justify-between pt-6 border-t border-white/5 opacity-40">
                                    ...
                                    {item.pinned && <CheckCircle2 className="w-3 h-3 text-secondary" />}
                                </div>
                            </motion.div>
                        ))}
                    </div>
                        ...
                        >
                            ...
                            <div className="p-8 md:p-12">
                                <div className="flex justify-between items-start mb-8">
                                    <div>
                                        <h3 className="text-2xl font-bold text-white mb-2 font-display">Deja tu <span className="text-primary italic">Susurro</span></h3>
                                        <p className="text-white/50 text-sm">Comparte tu luz con otros buscadores.</p>
                                    </div>
                                    <button
                                        onClick={() => setShowForm(false)}
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] uppercase tracking-widest font-bold text-white/40 ml-4">Tu Nombre</label>
                                        <input
                                            required
                                            type="text"
                                            ...
                                            className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder:text-white/20 focus:outline-hidden focus:border-primary/50 transition-all"
                                        />
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-[10px] uppercase tracking-widest font-bold text-white/40 ml-4">Valoración</label>
                                        <div className="flex gap-4 bg-white/5 border border-white/10 rounded-2xl px-6 py-4 justify-center">
                                            {[1, 2, 3, 4, 5].map((star) => (
                                                <button
                                                    key={star}
                                                    type="button"
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] uppercase tracking-widest font-bold text-white/40 ml-4">Tu Experiencia</label>
                                        <textarea
                                            required
                                            rows={4}
                                            ...
                                            className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white placeholder:text-white/20 focus:outline-hidden focus:border-primary/50 transition-all resize-none"
                                        />
                                    </div>
